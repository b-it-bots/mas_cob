...
import mas_cob_move_base_action.msg
from mas_cob_move_base_action.msg import MoveBaseSafeFeedback, MoveBaseSafeResult

class SetupMoveBase(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes=['succeeded', 'failed'],
                             input_keys=['move_base_goal'],
                             output_keys=['move_base_feedback', 'move_base_result', 'pose'])

    def execute(self, userdata):
        # get base goal from actionlib
        base_goal = userdata.move_base_goal.destination_location
        userdata.pose = base_goal

        feedback = MoveBaseSafeFeedback()
        feedback.current_state = 'MOVE_BASE'
        feedback.text = '[move_base] moving the base to ' + base_goal
